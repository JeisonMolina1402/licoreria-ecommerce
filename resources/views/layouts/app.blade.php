...
<head>
    ...
    <style>
        .sidebar .nav-link, .offcanvas .nav-link {
            transition: all 0.3s ease; color: #adb5bd !important; margin-bottom: 5px; border-radius: 8px;
        }
        .sidebar .nav-link:hover, .offcanvas .nav-link:hover {
            background-color: #495057 !important; color: #ffffff !important; transform: translateX(5px);
        }
        .nav-link.active-menu {
            background-color: #6c757d !important; color: #ffffff !important; font-weight: bold;
        }
        .menu-estatico::-webkit-scrollbar { width: 6px; }
        .menu-estatico::-webkit-scrollbar-thumb { background-color: #6c757d; border-radius: 8px; }
        
        .topbar-icon {
            color: #212529 !important; transition: all 0.3s ease-in-out;
        }
        .topbar-icon:hover {
            color: var(--color_primario, #f39c12) !important; transform: scale(1.08) translateY(-2px);
        }
    </style>
...
<body class="bg-light">
    <div class="container-fluid p-0">
        <div class="row g-0">
            ...
            <div class="col-12 d-md-none bg-dark p-3 d-flex justify-content-between align-items-center sticky-top shadow-sm">
                <img src="{{ asset('images/logos/logo.png') }}" alt="Licorería Admin" style="height: 40px; filter: brightness(0) invert(1);">

                <div class="d-flex align-items-center gap-4">
                    <!-- CAMPANA MÓVIL -->
                    <div class="dropdown">
                        <button class="btn btn-link text-white position-relative border-0 p-0 text-decoration-none d-flex align-items-center btn-campana-notificaciones" type="button" data-bs-toggle="dropdown" aria-expanded="false">
                    ...
                    <button class="btn btn-outline-light btn-sm" type="button" data-bs-toggle="offcanvas" data-bs-target="#menuMovil">
            ...
            <nav class="col-md-2 d-none d-md-block bg-dark sidebar" style="min-height: 100vh;">
                <div class="position-sticky pt-4 menu-estatico" style="top: 0; height: 100vh; overflow-y: auto; overflow-x: hidden;">
                    <div class="text-center mb-4 px-3 mt-3">
                        <a href="{{ route('home') }}"><img src="{{ asset('images/logos/logo.png') }}" class="img-fluid" style="max-height: 100px; filter: brightness(0) invert(1);"></a>
                    </div>
                    ...
                    <ul class="nav flex-column text-white px-2 pb-5">
                        ...
                        @can('ver auditoria')<li class="nav-item mb-2"><a class="nav-link {{ request()->routeIs('auditoria.index') ? 'active-menu' : 'text-white' }}" href="{{ route('auditoria.index') }}"><i class="fa-solid fa-shield-halved me-2"></i> Auditoría</a></li>@endcan
                        ...
                        @can('gestionar reportes')<li class="nav-item mb-2"><a class="nav-link {{ request()->routeIs('reportes.index') ? 'active-menu' : 'text-white' }}" href="{{ route('reportes.index') }}"><i class="fa-solid fa-file-invoice me-2"></i> Reportes</a></li>@endcan
                            ...
                            <li class="nav-item mb-2 mt-2">
                                ...
                                <div class="collapse {{ request()->routeIs('usuarios.*') || request()->routeIs('roles.*') || request()->routeIs('permisos.*') ? 'show' : '' }}" id="collapseSeguridad">
                                    <ul class="nav flex-column ms-3 mt-1 pb-1" style="border-left: 2px solid #495057;">
                                        @can('gestionar usuarios')<li class="nav-item mb-1"><a class="nav-link py-1 {{ request()->routeIs('usuarios.*') ? 'text-white fw-bold' : 'text-muted' }}" href="{{ route('usuarios.index') }}"><i class="fa-solid fa-users me-2 small"></i> Usuarios</a></li>@endcan
            ...
            <div class="offcanvas offcanvas-start bg-dark text-white" tabindex="-1" id="menuMovil">
                <div class="offcanvas-header border-bottom border-secondary position-relative justify-content-center py-3">
                    <img src="{{ asset('images/logos/logo.png') }}" alt="Licorería Menu" style="height: 50px; filter: brightness(0) invert(1);">
                ...
                <div class="offcanvas-body d-flex flex-column px-0 pt-0">
                    <div class="bg-dark border-bottom border-secondary p-3 mb-3 shadow-sm">
                        <div class="d-flex align-items-center mb-3 pb-3 border-bottom border-secondary">
                            ...
                            <div style="line-height: 1.2;">
                                ...
                                <span class="text-white-50 small">{{ Auth::user()->email ?? 'admin@licoreria.com' }}</span>
                            </div>
                        </div>

                        <div class="d-flex flex-column gap-1">
                            <a href="{{ route('tienda.index') }}" class="btn btn-dark text-start border-0 text-white-50 w-100 px-2 py-2"><i class="fa-solid fa-store me-2"></i> Ver Tienda Pública</a>
                    ...
                    <h6 class="text-white-50 px-3 mb-2 mt-1" style="font-family: 'Cinzel', serif; font-size: 0.85rem; letter-spacing: 1px;"><i class="fa-solid fa-layer-group me-2"></i> MÓDULOS</h6>
            ...
            <main class="col-md-10 bg-light p-4" style="min-height: 100vh;">
                ...
                <div class="d-flex flex-column flex-md-row justify-content-between align-items-start align-items-md-center pb-3 mb-4 border-bottom gap-3">
                    <div>
                        <h2 class="h4 h3-md text-dark mb-0 fw-bold">@yield('titulo_modulo', 'Panel de Control')</h2>
                        <small class="text-muted">@yield('subtitulo_modulo')</small>
                    </div>

                    <div class="d-none d-md-flex align-items-center flex-wrap gap-4 w-100 justify-content-end w-md-auto">
    ...
    <div id="alertas-globales" data-success="{{ session('success') }}" data-error="{{ session('error') }}" data-validacion="{{ $errors->any() ? 'true' : 'false' }}" class="d-none"></div>
